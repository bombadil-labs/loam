...
| 4. Suppression, governed reads | `negationPredicate`, `negationWitnesses`, `governedDeltas`, `applyPolicy` merged (#48); vectors, governed loaders and the lens binding (schema tier) still to come | seams `negatedAt`, `lawfulSnapshot(now)` (#588, #589, #592); history reads split out; timed recordings and reader audit (#587); the swap landed (this PR); three caches and the raw strike walks remain | in progress |
