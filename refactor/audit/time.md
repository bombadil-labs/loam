...
- **Substrate gap:** SPEC-5 §2 says every order must end in `lexById`. SPEC-4 §114 says arrival order must not reach content. The channel reader may leak arrival order on ties. Ties can happen across restarts (entry 1) or between two processes that share a seed.
...
- **Class:** a possible Loam bug in the channel reader only, with a candidate in the resolve tier. `latestByKey` is correct. **Tier:** resolve. **Confidence:** CONFIRMED that `latestByKey` is correct, by recording (`recordings/out/time.latest-tie.json`: same winner in both orders). PLAUSIBLE that the channel reader leaks arrival order; it is not recorded yet.
...
- **The channel status reader** around `src/federation/channel.ts:505-570`. A possible arrival-order tie. Use two ingest orders.
