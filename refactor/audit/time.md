...
- **Class:** Loam policy (a bug), with a candidate in the resolve tier. **Tier:** resolve. **Confidence:** CONFIRMED for `latestByKey` by recording (`recordings/out/time.latest-tie.json`: same winner in both orders). PLAUSIBLE for the channel reader, which is not recorded yet.
