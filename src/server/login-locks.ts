// The failed-login limiter's state, in a file (SPEC §36 phase 9).
//
...
//
// A NON-REGULAR-FILE PATH IS REFUSED BEFORE ANY READ, and that is not tidiness — it is the fix for
// T120. `readFileSync` on a FIFO BLOCKS until someone writes the other end, and this module reads on
// the login door's hottest path, in a single-threaded loop. So a FIFO planted at login-locks.json by
// anyone who can reach the home would park the whole login door forever, for every name. `statSync`
// does not open the path, so it cannot block; a path that is not a regular file reads as no records,
// exactly as damaged bytes do. The unreachable budget is the fail-open cost; the parked door is not
// an acceptable one.
//
...
/**
 * Every record the home holds. An absent, damaged, or NON-REGULAR-FILE path reads as none — see the
 * header. The `statSync` is the T120 guard: it does not open the path, so a FIFO here cannot park the
 * login door, and a path that is not a plain file is refused before `readFileSync` can block on it.
 */
export function readLocks(home: string): Map<string, FailureRecord> {
  ...
  // REFUSE A NON-REGULAR FILE WITHOUT OPENING IT (T120). `readFileSync` on a FIFO blocks forever, in
  // the single-threaded loop the login door runs on — so a stat that never opens the path is what
  // stands between a planted FIFO and a parked door. Absent or unstattable (a dangling symlink) reads
  // as no records, the same fail-open direction as damaged bytes.
  let regular: boolean;
  try {
 ...
 *
 * A PURE FUNCTION OF THE COUNT, never of the clock. The door sleeps exactly this many milliseconds,
 * so no wall-clock step — forward or backward — can shorten or inflate a wait in flight. A deadline
 * computed against `lastFailureAt` would not have that property.
 *
 ...
 * `0, 200, 400, 800, 1600`. That boundary is railed in login-delay.test.ts, both sides.
 *
