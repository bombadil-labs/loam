 ...
 *
 * `verifyPassword` derives at the parameters the ENTRY carries. So a decoy at the DOOR's configured cost
 * would take a different time from a real miss the moment those two differ — and that difference is a
 * username oracle measured with a stopwatch instead of a status code. The decoy therefore borrows an
 * existing entry's parameters, and falls back to the door's own only when there is no entry to imitate, in
 * which case no name exists and there is nothing to tell apart.
 *
 * WHAT THIS DOES NOT CLOSE, stated rather than implied: it borrows ONE entry's parameters, so if the
 * entries in a file disagree, every name whose cost differs from the borrowed one is still distinguishable
 * by time. Uniform parameters across the file are the invariant the no-oracle property rests on — which
 * the shipped write path maintains, since `loam user create` always writes `DEFAULT_SCRYPT`. A file that
 * has been edited by hand, or written by a caller passing its own `scrypt`, can break it; `paramsDisagree`
 * below is how the door notices and says so. Closing it properly wants a constant-time floor on the
 * response, or a re-hash on login when an entry's cost is stale. Both are their own ticket.
 */
...
/**
 * Do this file's entries disagree about scrypt cost? A `true` here means the decoy hash can only imitate
 * one of them, so the login door's timing tells an attacker which names cost something else — see
 * `decoyParamsFor`. Reported rather than repaired, because repairing it means re-hashing a password the
 * door does not have.
 */
