...
//
// A wall-posture slate is refused at the door, and the refusal is load-bearing rather than
// stylistic: a wall pays real byte copies, so a wall slate would hold a SECOND copy of every
...
function slateStateDefect(claims: Claims, reactor: Reactor, operator: string): string | undefined {
  ...
  if (rec.posture !== "property" || rec.trust !== "curated") {
    return (
      `a slate must name a curated/property container — "${container}" is declared ` +
      `${rec.trust}/${rec.posture}. A WALL slate would hold a SECOND COPY of every condemned ` +
...
export async function cutImpl(
  ...
  // Correction 2 closes for every other wall: `unreachableWallReport` returns a wall as a FAULT only
