...
export interface GatherSpec {
  ...
  /**
   * Gather only what THIS author asserted. The mask governs whose STRIKES bind; this governs whose
   * CLAIMS count at all — a different question, and the one a fact only the operator may state has to
   * ask. Without it the select admits every author with write standing, and a resolution that picks
   * the latest claim hands the field to whoever wrote last.
   *
   * Omit it for ordinary data, where any author with standing may speak and the reader's lens decides
   * who is heard.
   */
