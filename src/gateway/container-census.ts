...
// predicate is the listing door's own: does any entity-pointer context appear in a surviving
// program's context union (read from the GROUPING, never the flat list — a superseded
...
// as vocabulary: a store's constitution is not soup.
//
...
export function containerCensusImpl(gw: Gateway, name: string): ContainerCensus {
  ...
  for (const d of members) {
    ...
    if (classifyClaim(d) === "data") {
      if (entityContexts(d).some((c) => union.has(c))) lit += 1;
      else dark += 1;
    } else {
      vocabulary += 1;
    }
