...
export async function eraseImpl(
  ...
  // holds outside this sweep, reported rather than silent. The tombstone's id and the target's
  // author ride out too: a cut collects them per member (§29.5) rather than re-deriving them from a
  // ground the purge just moved.
  return {
    erased: id,
    citations,
    kept: walls.kept,
