...
// lens it reads; `version` an optional §17 vN pin (absent = the latest lens); `consumes` the fields it
// reads (checked at push so it never references what the lens cannot fill, §23.4); `bundle` the runnable
// ESM (`export default (node) => html`). Inline for v1 — the snapshot doctrine's first rung.
...
// A surviving renderer binding, as the host receives it, plus its true name (the delta's content address)
// and its ground-order timestamp.
export interface RendererBinding extends RendererSpec {
...
// Serialize a renderer binding's claims: the `renders` key (files under `renderer:<route>`), and the
// route / schema / version / consumes / bundle as primitives. No definitions travel here — a renderer
    ...
    pointers: [
      {
        ...
        target: {
          kind: "entity",
          entity: { id: rendererEntity(spec.route), context: CTX_RENDERER },
        },
      },
      { role: "route", target: { kind: "primitive", value: spec.route } },
      { role: "schema", target: { kind: "primitive", value: spec.schemaName } },
      ...(spec.version === undefined
        ? []
        : [{ role: "version", target: { kind: "primitive" as const, value: spec.version } }]),
      { role: "consumes", target: { kind: "primitive", value: JSON.stringify(spec.consumes) } },
      { role: "bundle", target: { kind: "primitive", value: spec.bundle } },
    ],
  };
}
...
export function readRenderers(reactor: Reactor, operator?: string): RendererBinding[] {
  ...
  for (const delta of lawful) {
    ...
    const versionRaw = primitive(delta.claims, "version");
