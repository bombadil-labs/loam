...
const COMMANDS: Readonly<Record<CommandName, CommandSpec>> = {
  ...
  tombstones: {
    ...
    notes: [
      ...
      "",
      "A STRUCK RECEIPT IS WITHDRAWN (§11): the record no longer stands, but the id stays refused",
  ...
  const withheld =
    ledger.inert <= 0
      ...
      : `\n  ${ledger.inert} more receipt${ledger.inert === 1 ? "" : "s"} in the ground ` +
        `${ledger.inert === 1 ? "does" : "do"} not bind — struck (withdrawn: the id stays refused, ` +
