...
describe("LocalStorageBackend: the seed key is structurally outside the delta set", () => {
  it("loam:<store>:seed is never returned by a read and never purged as a delta", async () => {
    ...
    // ...and `holds` does not see it either (ticket T67): the key EXISTS in storage, so a probe
    // that fell through to `getItem(keyFor("seed"))` would answer true — and an erase verdict
    // would then report key material as retained subject content. The seed is not a delta; a
    // byte-presence question about deltas must not be answerable by it.
