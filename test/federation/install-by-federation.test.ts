...
//
// The gateway-level half. Its siblings hold the rails whose cost is a process: `-cli` drives the
// shipped command against sqlite homes, `-resolvers` drives the second act. THEY ARE SEPARATE FILES
// BECAUSE OF THE CLOCK, not the subject: one file of all three ran fifty seconds serially and
// starved its own tests under a loaded suite, which is a flake — and a flake teaches whoever reads
// a red bar to re-run instead of look. The shared fixtures live in `t209-fixtures.ts`.
//
...
// test/gateway/probation-frame.test.ts owns the frame's own contract). (2) That a COLD process
// serves a blessed app: the served bundle is loaded by `prepareRoute`, which the HTTP door calls
// before every render, and these rails run in one process where the publish already warmed the ESM
// cache. A REBOOT DOES NOT CLOSE IT EITHER: the ESM cache is a module-level Map that `close()` never
// clears, so any rail that blesses and then reboots in ONE process warms it before the reboot.
// Closing this wants the blessing to happen in a DIFFERENT process — a CLI child — or an eviction
