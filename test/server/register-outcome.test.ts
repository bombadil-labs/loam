...
// serves it. Same H7 shape the CLI's `register` carried; these are its two remaining doors.
//
// Two-sided at both levels, per door: an unbindable registration answers `bound: false` WITH the
// proximate cause, AND its type really does not answer on the surface — while an ordinary
// registration answers `bound: true`, carries no reason, and its type answers immediately.
//
// GAP, named: `POST /admin/register` (the HTML panel) discards the outcome the same way and is
// NOT covered here — it needs a rendered page to say it in, which is a page change rather than a
// field. The rail that would close it is an admin-page assertion; this file does not pretend to.
...
describe("the registration doors report whether anything now serves the name", () => {
  it("POST /:mount/register: a rival body is 200, persisted, and says bound: false with the cause", async () => {
    ...
    // Object level: what the door SAID matches what the surface DOES. The manual Plant still
    // serves its own shape — the published rival shapes nothing.
    const answer = await gql(`{ plant(entity: "${FERN}") { _hex } }`);
