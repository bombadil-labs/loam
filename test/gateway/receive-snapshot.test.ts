...
describe("exact receiving snapshot operands", () => {
  ...
  it("two pauses that list the same members in another order are one pause, not a conflict", () => {
    const a = law();
    const permuted = pause(a, {
      selection: { ...selection(a), memberIds: [...selection(a).memberIds].reverse() },
    });
    const result = run(a, [binding, pause(a), permuted])[0]!;
    expect(result.status).toBe("selected");
    expect(result.registration!.boundId).toBe(a[3]!.id);
  });
