...
describe("§30 criterion 32d: whose credentials run, and where it appears", () => {
  ...
  it("appears in the emitted page ABOVE the onboarding copy, and in the pack door's response", async () => {
    const gw = await ready({ writable: ["height"] });
    const { page, capability } = gw.packArtifact("plant", FERN, { server: "My Loam" });
    expect(capability).toEqual(statementOf(gw));
    for (const line of capability) {
      // Rendered as escaped HTML, so compare on a distinctive fragment rather than the whole line.
      expect(page).toContain(line.split("“")[0]!.slice(0, 40));
    }
    // The page is inert until a connector exists, so "connect your store" IS the acceptance gesture
    // and the statement sits in front of it.
