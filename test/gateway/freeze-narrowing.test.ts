...
// `test/gateway/narrowing.ts` exists for. These rails ask the resolved question at both levels
// (delta and object) rather than set membership, which is the weaker shape that passed while the
// original bug shipped:
//
//   - PRESERVES SUPPRESSION: a claim struck at the source must never read as LIVE in a store
//     loaded from the version's members.
...
//
// Each case also asserts the OBJECT level — what `resolvedNode` answers at the destination —
// because members and what a reader sees come apart exactly when suppression is involved.
// Deliberately NOT asserted here: the address properties (order-freedom, sensitivity, non-drift);
// those are `container-identity.test.ts`'s rails.
...
// A consumer loading the version: a fresh store holding ONLY the version's members — the
// destination side of the narrowing, where a stranded strike would read as live.
