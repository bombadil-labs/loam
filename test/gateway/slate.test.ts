...
// matters: a wall-posture slate is refused BY CONSTRUCTION (it would hold a second copy of every
...
//
// WHAT THIS FILE DELIBERATELY DOES NOT ASSERT: the three closures (slate-doors.test.ts), the cut
// (slate-cut.test.ts), the graveyard arithmetic (graveyard.test.ts), and the receipt
// (slate-receipt.test.ts). Two-sidedness is kept per rail here as everywhere in T64: every rail that
// proves something is refused or removed also names a live bystander that still answers.
...
describe("T64 criterion 1 — a slate is a curated PROPERTY container plus a record", () => {
  it("a wall-posture slate is REFUSED at the door, naming both knobs", async () => {
    ...
    await expect(
      standSlate(gw, {
        container: "container:slate:wall",
        members: [condemned],
        closes: ["egress", "cite"],
        posture: "wall",
      }),
    ).rejects.toThrow(/curated\/property/);
    // The refusal names BOTH knobs, so a reader learns which one to change.
    await expect(
      standSlate(gw, {
        container: "container:slate:wall2",
        members: [condemned],
        closes: ["egress", "cite"],
        posture: "wall",
      }),
    ).rejects.toThrow(/curated\/wall/);
  ...
  it("an UNTRUSTED slate is refused too — untrusted cannot take posture property (§28.3)", async () => {
    const gw = await bootSlateStore();
    const condemned = observed(FERN, "height", 30, 1000, OP_SEED);
    ...
    ).rejects.toThrow(/untrusted.*cannot take posture "property"/s);
    await gw.close();
  });
  ...
  it("with a slate standing, an ordinary erase(unrelated) still COMPLETES", async () => {
    ...
    // wall-posture slate WOULD — every subsequent erase would refuse while the slate stood.
    const gw = await bootSlateStore();
    const condemned = observed(FERN, "height", 30, 1000, OP_SEED);
...
describe("T64 criterion 3 — `closes` and `deadline` are required, and `none` is sayable", () => {
  ...
  it("`closes: none` is ACCEPTED and closes nothing — every door still serves", async () => {
    ...
    // Egress still offers it; the append door still admits a citation; the read door still serves.
    expect(gw.offeredDeltas().map((d) => d.id)).toContain(condemned.id);
...
describe("T64 criterion 15 — forgiveness, both sides of the cut", () => {
  ...
  it("AFTER the cut: striking a tombstone permits the id's return but restores no bytes", async () => {
    ...
    expect(await gw.backend.holds(condemned.id)).toBe(false);
    // The re-derived receipt reports FORGIVEN with its strike id, not still-forgotten.
