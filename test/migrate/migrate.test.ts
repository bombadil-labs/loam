...
describe("migration: 0.2 → 0.3 schema-definition vocabulary", () => {
  ...
  it("is idempotent by content address: re-migrating adds no new deltas", () => {
    const once = migrate(oldStore, { seed }).deltas;
    const twice = migrate(once, { seed }).deltas;
    expect(new Set(twice.map((d) => d.id))).toEqual(new Set(once.map((d) => d.id)));
  });
});
