...
// Symlinks and gitlinks are tracked but not readable as content — the repo tracks a `node_modules`
...
describe("no tracked source file smuggles a raw NUL byte", () => {
  it("every tracked file is NUL-free, except the ones that are binary by design", () => {
    const files = trackedFiles();
    // A guard that scanned nothing would pass loudest of all.
    ...
    expect(
      ...
      offenders.length === 0
        ? ""
        : `these tracked files hold raw 0x00 bytes, which makes them BINARY to git — their diffs ` +
