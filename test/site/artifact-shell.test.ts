...
import { SEALED_CHANNELS, sealRealm } from "../../src/gateway/artifact-realm.js";
...
describe("the page's TWO manglings agree with the store's, function for function", () => {
  // The page is standalone — it cannot import from the gateway — so it carries its own copies of the two
  // derivations the store applies. That duplication is only safe while somebody pins them together, and
  // a STUB cannot: `answerFor` echoes back whatever field the document names, so it agrees with any
  // spelling. Hence this: extract the page's own functions and compare them to the store's exports.
  const shellFns = (
    page: string,
    ...
    return evalPageValue(`${m![0]}; return { legalName: legalName, rootField: rootField };`);
  };

  it("both agree over every shape, and DIVERGE on a capital initial", async () => {
    const gw = await boot();
    ...
    // The divergence is the whole reason there are two of them: a rail over lowercase-initial names
    // only would pass with one function doing both jobs, and the WRITE site would be silently dead.
